...
class InvoiceGenerator:
    ...
    def __init__(self, root):
        ...
        # Fixed: Changed pack() to place() since x, y, width, height require place()
        ...
        self.company_name = Entry(
            self.frame, font=("times new roman", 15), bg="light grey"
        )
        ...
        Button(
            ...
            cursor="hand2",
            bg="#B00857",
        ).place(x=50, y=640, width=180, height=40)

    def browse(self):
        self.file_name = filedialog.askopenfilename(title="Select a File")
        Label(
            self.frame,
            text=os.path.basename(self.file_name),
            font=("times new roman", 15),
        ).place(x=270, y=600)

    def generate_invoice(self):
        c = canvas.Canvas(
            "Invoice by DataFlair.pdf", pagesize=(200, 250), bottomup=0
        )
